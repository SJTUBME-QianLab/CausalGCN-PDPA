...
class FeederGraph:
    def __init__(self, emb_name, fold, split_seed, out_dir, mode, graph_arg, debug=False, save=False):
        assert mode in ['train', 'test']
        self.fold_num = 5
        self.split_seed = split_seed
        self.fold = fold
        ...
        self.pxs, self.label, self.sample_name = pp.train_test_split(seed=split_seed, fold=fold, mode=mode)
        self.N, self.P = self.pxs.shape[:2]
        self.coordinates = np.load(os.path.join(self.out_dir, 'coordinates.npy'))
        coord = self.coordinates[None].repeat(self.N, axis=0)  # [P,3]->[B,P,3]

        # construct graph
        graph = ConstructGraph(**self.graph_arg)
        self.node, self.edge = graph.construct(self.pxs, self.embedding, coord)
        assert self.node.shape[1] == self.P and self.node.shape[0] == self.N
        self.dim = self.node.shape[2]
