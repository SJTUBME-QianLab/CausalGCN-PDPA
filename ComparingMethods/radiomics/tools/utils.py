...
def classic_clf(data_dict):
    X_tr_tmp, X_te_tmp, Y_train, Y_test = \
        data_dict['x_train_eval'], data_dict['x_test'], data_dict['y_train_eval'], data_dict['y_test']
    ...
    all_eval = evaluate_pred(pred, Y_test)
    return dict(zip(eval_name, all_eval))
...
def get_auc_cl2(true, prob, softmax=True):
    num_class = int(max(true) + 1)
    assert num_class == 2
    if isinstance(prob, pd.DataFrame):
        prob = prob.values
